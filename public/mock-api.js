...
const api = {
	 ...
	 * @property {string|undefined} majorCategory the major category to filter
	 * @property {string|undefined} subCategory the sub category to filter
	 * @returns
	 */
	getItems: async function (opt) {
		...
		let items = flat(this.data.Shop, []).slice(start, end);
		if (opt.majorCategory) {
			items = items.filter(item => item.category === opt.majorCategory);
		}
		if (opt.subCategory) {
			items = items.filter(item => item.subCategory === opt.subCategory);
		}
		return items;
	},
    /**
